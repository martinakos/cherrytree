                                        ...
                                        int sel_end)
{
    ...
    if (tree_iter.get_node_is_text()) {
        ...
        for (size_t i = 0; i < html_slots.size(); ++i) {
            ...
            if (i < widgets.size()) {
                try {
                    if (dynamic_cast<CtHorizLine*>(widgets[i]))
                        node_html_text += "<hr/>";
                    else if (auto embfile = dynamic_cast<CtImageEmbFile*>(widgets[i]))
                                                      ...
                                                      const std::list<CtAnchoredWidget*>* pCellWidgets/*=nullptr*/)
{
    ...
    if (syntax_highlighting == CtConst::RICH_TEXT_ID) {
        ...
        for (CtAnchoredWidget* widget : widgets) {
            ...
            if (dynamic_cast<CtHorizLine*>(widget)) node_html_text += "<hr/>";
            else if (CtImage* image = dynamic_cast<CtImage*>(widget)) node_html_text += _get_image_html(image, tempFolder, images_count, nullptr, false/*single_file*/);
